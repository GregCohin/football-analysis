...
function PrintableSheetsTab({ causerie, updateCauserie, roster, gameplan }) {
    ...
    return (
      <div>
        ...
        {selectedSheet.title === "Composition" && (
          <button className="btn btn-ghost btn-small no-print" onClick={() => placeBySystem(selectedSheet.id)} style={{ marginBottom: 10 }}>
  ...
  return (
    <div>
      ...
      {causerie.sheets.length > 0 && (
        <button className="btn btn-ghost btn-small no-print" onClick={() => setShowAllPrint(true)} style={{ marginBottom: 16 }}>
    ...
    <div className="stats-screen">
      ...
      {selected && (
        <div>
          ...
          {subTab === "fiches" && <PrintableSheetsTab causerie={selected} updateCauserie={updateCauserie} roster={roster} gameplan={gameplan} />}
          {subTab === "notes" && (
            <div>
...
function NextMatchCard({ fixtures, setSection }) {
...
function CompetitionsScreen({ setSection }) {
  ...
  function createCompetition() {
    if (typeTab === "championnat") {
      ...
      const instance = { ...emptyCompetitionInstance(levelLabel, typeTab, newGroupName.trim()), level: newLevel };
      setData((prev) => {
    ...
    <div className="stats-screen">
      ...
      {selected && (
        <div style={{ marginTop: 16 }}>
          ...
          <NextMatchCard fixtures={selected.fixtures} setSection={setSection} />
      ...
      {reportsSubTab === "mes" && (
        <>
          ...
          {allFullMatches.length > 1 && (
            <button className="btn btn-ghost btn-small no-print" onClick={() => { setCompareMode((v) => !v); setCompareMatchIds([]); }} style={{ marginBottom: 14 }}>
          ...
          {selectedClub && (
            <div style={{ marginTop: 16 }}>
              ...
              {clubFeaturedMatch && clubReport && clubSignals && (
                <MatchReportView match={clubFeaturedMatch} report={clubReport} signals={clubSignals} labelA={selectedClub} labelB={otherTeamInClubMatch} />
              )}
...
function AccueilScreen({ matches, roster, setSection }) {
  ...
  return (
    <div className="stats-screen">
      ...
      <div className="panel-heading" style={{ marginTop: 28 }}>À la une</div>
      <div className="dashboard-cards">
        <div className="dashboard-card" onClick={() => setSection("competitions")}>
          <div className="dashboard-card-title">Prochain match</div>
          {upcomingFixture ? (
            <>
              <div className="dashboard-card-main">vs {upcomingFixture.opponent}</div>
              <div className="dashboard-card-meta">{formatDateFr(upcomingFixture.date)} · {upcomingFixture.venue}</div>
              <button
                className="btn btn-ghost btn-small"
                style={{ marginTop: 8 }}
                onClick={(e) => {
                  e.stopPropagation();
                  try { localStorage.setItem("tf_causerie_seed", JSON.stringify({ opponent: upcomingFixture.opponent, date: upcomingFixture.date })); } catch (err) {}
                  setSection("causerie");
                }}
              >
                Préparer la causerie
        ...
        <div className="dashboard-card" onClick={() => setSection("competitions")}>
          <div className="dashboard-card-title">Dernier match joué</div>
          {lastPlayedFixture ? (
            <>
              <div className="dashboard-card-main">{lastPlayedFixture.ourScore} - {lastPlayedFixture.theirScore} vs {lastPlayedFixture.opponent}</div>
              <div className="dashboard-card-meta">{formatDateFr(lastPlayedFixture.date)}</div>
            </>
          ) : <div className="gameplan-empty">Aucun résultat enregistré</div>}
        </div>

        <div className="dashboard-card" onClick={() => setSection("sessions")}>
          <div className="dashboard-card-title">Prochaine séance</div>
          {upcomingSession ? (
            <>
              <div className="dashboard-card-main">{upcomingSession.name}</div>
              <div className="dashboard-card-meta">{formatDateFr(upcomingSession.date)}</div>
            </>
          ) : <div className="gameplan-empty">Aucune séance planifiée</div>}
          {upcomingEnjeux && <div className="dashboard-card-meta" style={{ marginTop: 6 }}>Enjeu du prochain match : {upcomingEnjeux}</div>}
        </div>

        <div className="dashboard-card" onClick={() => setSection("sessions")}>
          <div className="dashboard-card-title">Dernière séance</div>
          {lastSession ? (
            <>
              <div className="dashboard-card-main">{lastSession.name}{lastSession.rating && lastSession.rating.coachScore ? ` — ${lastSession.rating.coachScore}/10` : ""}</div>
              <div className="dashboard-card-meta">{formatDateFr(lastSession.date)}</div>
            </>
          ) : <div className="gameplan-empty">Aucune séance passée</div>}
        </div>

        <div className="dashboard-card" onClick={() => setSection("medical")}>
          <div className="dashboard-card-title">Alertes du jour</div>
          <div className="dashboard-card-main">{missingHooper.length} joueur{missingHooper.length > 1 ? "s" : ""} sans Hooper aujourd'hui</div>
          <div className="dashboard-card-meta">{activeInjuries.length} blessure{activeInjuries.length > 1 ? "s" : ""} en cours{activeInjuries.length > 0 ? ` (${activeInjuries.map((i) => { const p = roster.find((r) => r.id === i.playerId); return p ? playerFullName(p) : "?"; }).join(", ")})` : ""}</div>
          {fatigueRisks.length > 0 && <div className="dashboard-card-meta">{fatigueRisks.length} joueur{fatigueRisks.length > 1 ? "s" : ""} en zone de vigilance (charge/fatigue)</div>}
        </div>

        <div className="dashboard-card" onClick={() => setSection("reports")}>
          <div className="dashboard-card-title">Dernier rapport de match</div>
          {lastReport && signals ? (
            <>
              <div className="dashboard-card-main">{lastReport.match.name}</div>
              <div className="dashboard-card-meta">
                {signals.positivesCollective[0] ? `+ ${signals.positivesCollective[0].text}` : "Pas assez de données pour un signal"}
                {signals.negativesCollective[0] ? ` · − ${signals.negativesCollective[0].text}` : ""}
              </div>
            </>
        ...
        <div className="dashboard-card" onClick={() => setSection("competitions")}>
          <div className="dashboard-card-title">Classement championnat</div>
          {standings.length > 0 ? (
            <div className="table-scroll">
              <table className="stat-table">
                <thead><tr><th>#</th><th>Équipe</th><th>Pts</th></tr></thead>
                <tbody>
                  {standings.slice(0, 5).map((t, i) => <tr key={t.id}><td>{i + 1}</td><td>{t.team}</td><td>{t.points}</td></tr>)}
                </tbody>
              </table>
            </div>
        ...
        <div className="dashboard-card" onClick={() => setSection("causerie")}>
          <div className="dashboard-card-title">Causerie</div>
          {nextCauserie ? (
            <>
              <div className="dashboard-card-main">{nextCauserie.name}</div>
              <div className="dashboard-card-meta">{formatDateFr(nextCauserie.date)}{nextCauserie.opponent ? ` · vs ${nextCauserie.opponent}` : ""}</div>
            </>
          ) : <div className="gameplan-empty">Aucune causerie à préparer</div>}
        </div>

        <div className="dashboard-card" onClick={() => setSection(alerts[0] ? alerts[0].section : "accueil")}>
          <div className="dashboard-card-title">Alertes</div>
          {alerts.length === 0 && <div className="gameplan-empty">Rien à signaler</div>}
          {alerts.slice(0, 4).map((a, i) => (
            <div key={i} className="dashboard-card-meta" style={{ marginTop: i === 0 ? 0 : 4 }}>{a.label}</div>
          ))}
          {alerts.length > 4 && <div className="dashboard-card-meta" style={{ marginTop: 4 }}>+ {alerts.length - 4} autre(s)</div>}
        </div>
      </div>
    </div>
  );
