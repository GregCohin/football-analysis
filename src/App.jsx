...
function emptyObjective(theme, setBy) {
  return { id: newId(), theme, setBy, title: "", description: "", targetDate: "", status: "En cours", progressNotes: "" };
}
...
function emptyCauserie(name, opponent, date) {
  return {
    ...
    selectedPlayerIds: [], playerNotes: {}, sheets: [], notes: "",
  };
}
...
function VideothequeScreen() {
  ...
  const filtered = clips.filter((c) => clipInSubTab(c, subTab));
  const grouped = {};
  filtered.forEach((c) => {
      ...
      <div className="tabs">
        {VIDEOTHEQUE_TABS.map((t) => (
          <button key={t.id} className={`tab ${subTab === t.id ? "active" : ""}`} onClick={() => setSubTab(t.id)}>
            {t.label} <span className="scouting-club">({clips.filter((c) => clipInSubTab(c, t.id)).length})</span>
          </button>
        ))}
      </div>
        ...
        return (
          <div key={phase.key} style={{ marginBottom: 18 }}>
            ...
            {blocks.map((b) => {
              ...
              return (
                <div className="scouting-card" key={b.id} style={{ marginBottom: 8 }}>
                  <div className="scouting-info">
...
function AccueilCalendar({ events, setSection }) {
    ...
    <div>
      ...
      <div className="calendar-grid">
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div key={i} className={`calendar-day ${isToday ? "today" : ""} ${selectedDate === ds ? "selected" : ""}`} onClick={() => setSelectedDate(ds === selectedDate ? null : ds)}>
...
function AccueilScreen({ matches, roster, setSection }) {
  ...
  return (
    <div className="stats-screen">
      ...
      <AccueilCalendar events={events} setSection={setSection} />
