...
function ClubCrossCalendarScreen({ teams }) {
  ...
  useEffect(() => {
    ...
    const result = teams.map((t) => {
      ...
      return { team: t, events: buildAccueilEvents(comp, sessions) };
    });
  ...
  return (
    <div>
      ...
      <div className="scouting-list" style={{ marginTop: 10 }}>
        ...
        {data.bookings.length === 0 && <div className="empty-state">Aucun créneau réservé pour l'instant.</div>}
      </div>
    </div>
  );
}
...
function buildAccueilEvents(comp, sessions) {
  const events = [];
  comp.competitions.forEach((c) => {
...
function WidgetMeteo() {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        fetch(`https://api.open-meteo.com/v1/forecast?latitude=${pos.coords.latitude}&longitude=${pos.coords.longitude}&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=auto&forecast_days=1`)
          .then((r) => r.json())
  ...
  return (
    <>
      ...
      {weather && (
        <>
          <div className="dashboard-card-main">{WEATHER_CODE_LABELS[weather.weathercode[0]] || "—"}</div>
          <div className="dashboard-card-meta">{Math.round(weather.temperature_2m_min[0])}° / {Math.round(weather.temperature_2m_max[0])}°</div>
        </>
      )}
    </>
  );
...
function AccueilScreen({ matches, roster, setSection }) {
  ...
  const events = buildAccueilEvents(comp, sessions);
  const allFixtures = comp.competitions.flatMap((c) => c.fixtures);
  const upcomingFixture = allFixtures.filter((f) => !fixtureIsPlayed(f)).sort((a, b) => new Date(a.date) - new Date(b.date))[0];
