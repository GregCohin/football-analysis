...
import { Play, Pause, ArrowLeft, X, Download, Video as VideoIcon, Film, Menu, Home, Target, Users, Trophy, BarChart, ClipboardList, FileText, Eye, Search, Activity, Megaphone, Shield, BookOpen, Sparkles } from "lucide-react";
...
function scopeSuffixFor(teamId, seasonId) {
...
function drawArrowHeadOnly(ctx, fromX, fromY, tipX, tipY) {
  const headLen = 14;
...
function drawArrowHead(ctx, x1, y1, x2, y2) {
  ...
  drawArrowHeadOnly(ctx, x1, y1, x2, y2);
}
...
function drawWavyArrow(ctx, x1, y1, x2, y2, curved, cx, cy) {
  const segments = 24;
  const amplitude = 4;
  ...
  ctx.stroke();
  drawArrowHeadOnly(ctx, x2 - Math.cos(baseAngle) * 14, y2 - Math.sin(baseAngle) * 14, x2, y2);
}
  ...
  return (
    <div>
      ...
      <div className={`pad-wrap ${halfPitch ? "pad-wrap-half" : ""}`} ref={wrapRef}>
        {halfPitch ? <PitchBackgroundHalf /> : <PitchBackgroundFull />}
        <canvas ref={canvasRef} onClick={handleClick} onMouseDown={handleMouseDown} onMouseMove={handleMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} style={{ cursor: isRotateTool ? "grab" : undefined }} />
        {textPending && (
...
function drawPadElement(ctx, el, w, h) {
  if (el.type === "arrowMove" || el.type === "arrowPass" || el.type === "arrowDribble") {
    const x1 = el.x1 * w, y1 = el.y1 * h, x2 = el.x2 * w, y2 = el.y2 * h;
    const cx = el.cx != null ? el.cx * w : null, cy = el.cy != null ? el.cy * h : null;
  ...
  if (def.shape === "disc") {
    ctx.beginPath(); ctx.arc(x, y, 11, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    if (el.number) { ctx.fillStyle = "#1a1a1a"; ctx.font = "bold 10px sans-serif"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(el.number, x, y); ctx.textAlign = "left"; ctx.textBaseline = "alphabetic"; }
  } else if (def.shape === "triangle") {
    ctx.beginPath(); ctx.moveTo(x, y - 9); ctx.lineTo(x + 8, y + 7); ctx.lineTo(x - 8, y + 7); ctx.closePath(); ctx.fill(); ctx.stroke();
  } else if (def.shape === "ball") {
    ctx.beginPath(); ctx.arc(x, y, 7, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ...
    ctx.beginPath(); ctx.moveTo(x - 10, y - 16); ctx.lineTo(x - 10, y + 16); ctx.moveTo(x + 10, y - 16); ctx.lineTo(x + 10, y + 16); ctx.stroke();
    for (let i = -14; i <= 14; i += 7) { ctx.beginPath(); ctx.moveTo(x - 10, y + i); ctx.lineTo(x + 10, y + i); ctx.stroke(); }
  } else if (def.shape === "pole") {
    ctx.fillStyle = def.color;
    ctx.fillRect(x - 2, y - 15, 4, 24);
    ctx.beginPath(); ctx.arc(x, y - 15, 4, 0, Math.PI * 2); ctx.fill();
  } else if (def.shape === "hurdle") {
    ctx.strokeStyle = def.color; ctx.lineWidth = 2.5;
    ctx.beginPath();
    ctx.moveTo(x - 12, y + 8); ctx.lineTo(x - 6, y - 6);
    ctx.moveTo(x + 12, y + 8); ctx.lineTo(x + 6, y - 6);
...
function findNearestRotatable(pos, elements, threshold = 0.05) {
...
function TacticalPadEditor({ elements, setElements }) {
  const wrapRef = useRef(null);
  const canvasRef = useRef(null);
  ...
  useEffect(() => { setPendingPoints([]); setPreview(null); }, [tool]);
  ...
  }, [elements, preview, size, pendingPoints, isZoneTool, arrowColor]);
  ...
  function handleClick(e) {
    const pos = getPos(e);
    if (isRotateTool) return; // géré au clic-glisse (mousedown/mousemove/mouseup)
    if (tool === "text") { setTextPending(pos); setTextValue(""); return; }

    if (isZoneTool) {
      if (pendingPoints.length === 0) { setPendingPoints([pos]); return; }
      setElements((prev) => [...prev, { id: newId(), type: "zone", x1: pendingPoints[0].x, y1: pendingPoints[0].y, x2: pos.x, y2: pos.y, color: "#E3B23C" }]);
      setPendingPoints([]); setPreview(null);
      return;
    }
    ...
    if (isArrowTool) {
      ...
      if (pendingPoints.length === 1) {
        if (!curveMode) {
          setElements((prev) => [...prev, { id: newId(), type: tool, x1: pendingPoints[0].x, y1: pendingPoints[0].y, x2: pos.x, y2: pos.y, curved: false, color: arrowColor }]);
          setPendingPoints([]); setPreview(null);
        } else {
          setPendingPoints([...pendingPoints, pos]);
        }
      ...
      setElements((prev) => [...prev, { id: newId(), type: tool, x1: pendingPoints[0].x, y1: pendingPoints[0].y, x2: pendingPoints[1].x, y2: pendingPoints[1].y, curved: true, cx: pos.x, cy: pos.y, color: arrowColor }]);
      setPendingPoints([]); setPreview(null);
      return;
    }

    const def = PAD_ELEMENT_TYPES.find((t) => t.key === tool);
    setElements((prev) => [...prev, { id: newId(), type: tool, x: pos.x, y: pos.y, color: def ? def.color : "#fff" }]);
  }
  ...
  function commitText() {
    if (textValue.trim()) setElements((prev) => [...prev, { id: newId(), type: "text", x: textPending.x, y: textPending.y, text: textValue.trim(), color: "#ffffff" }]);
    setTextPending(null);
  }
  ...
  return (
    <div>
      ...
      <p className="hint">Choisis un élément puis clique sur le terrain pour le placer. Pour les flèches et la zone, un clic par étape (coche "Courber" avant de commencer pour ajouter un point de courbure). "Flèche course" est en pointillés, "Flèche passe" en trait plein, "Flèche course + ballon" est ondulée. L'outil "Pivoter" oriente un élément déjà placé au clic-glisse.</p>
    </div>
  );
}
...
function genVs(nbA, nbB, opts) {
  opts = opts || {};
  const els = [];
  const spreadY = (n, i) => (n <= 1 ? 0.5 : 0.18 + (0.64 * i) / (n - 1));
  const baseXA = opts.xA != null ? opts.xA : 0.22;
  const baseXB = opts.xB != null ? opts.xB : 0.55;
  for (let i = 0; i < nbA; i++) els.push(pel("playerA", baseXA, spreadY(nbA, i), { number: i + 1 }));
  for (let i = 0; i < nbB; i++) els.push(pel("playerB", baseXB, spreadY(nbB, i), { number: i + 1 }));
  if (opts.hasGoal) els.push(pel("goal", 0.94, 0.5));
...
function genLine(nbDef, nbAtt) {
  const els = [];
  const spreadY = (n, i) => (n <= 1 ? 0.5 : 0.15 + (0.7 * i) / (n - 1));
  for (let i = 0; i < nbDef; i++) els.push(pel("playerB", 0.35, spreadY(nbDef, i), { number: i + 1 }));
  for (let i = 0; i < nbAtt; i++) els.push(pel("playerA", 0.65, spreadY(nbAtt, i), { number: i + 1 }));
  els.push(pel("keeper", 0.1, 0.5));
  els.push(pel("goal", 0.06, 0.5));
...
function genFreeKick(zone) {
  ...
  if (zone === "off") {
    ...
    els.push(pel("keeper", 0.94, 0.5)); els.push(pel("goal", 0.97, 0.5));
  } else {
    els.push(pel("keeper", 0.06, 0.5)); els.push(pel("goal", 0.03, 0.5));
...
function genPenalty() {
  return [pel("ball", 0.79, 0.5), pel("playerA", 0.7, 0.5, { number: 1 }), pel("keeper", 0.94, 0.5), pel("goal", 0.97, 0.5)];
}
...
const STARTER_EXERCISES = [
  ...
  },
  {
    ...
    diagram: genVs(6, 6, { zone: [0.05, 0.05, 0.55, 0.95] }),
  },
  {
...
function emptyExerciseForm() {
  return { name: "", objectif: "", duree: 15, nbJoueurs: "", materiel: "", description: "", gameplanSection: "", gameplanChoice: "", diagram: [] };
}
...
function ExerciseFormPanel({ form, setForm, onSave, onCancel, gameplan }) {
  ...
  return (
    <div className="new-match-card">
      ...
      <div className="roster-form-section-title">Lien avec le Projet de jeu (optionnel)</div>
      <label>
        Question travaillée
      ...
      <TacticalPadEditor elements={form.diagram} setElements={(v) => setForm((f) => ({ ...f, diagram: typeof v === "function" ? v(f.diagram) : v }))} />
...
function ExerciseCreationScreen({ exercises, setExercises, gameplan }) {
  ...
  function populateStarterExercises() {
    ...
    const toAdd = STARTER_EXERCISES.filter((ex) => !existingNames.has(ex.name)).map((ex) => ({ id: newId(), ...emptyExerciseForm(), ...ex, createdAt: Date.now() }));
    ...
    alert(`${toAdd.length} exercice${toAdd.length > 1 ? "s" : ""} ajouté${toAdd.length > 1 ? "s" : ""}, un par thème du Projet de jeu — à ajuster librement selon tes propres choix déclarés.`);
  }

  function updateStarterDiagrams() {
    const starterByName = {};
    STARTER_EXERCISES.forEach((ex) => { starterByName[ex.name] = ex.diagram; });
    let updated = 0;
    ...
    <div>
      {!showForm && (
        <div className="home-actions-row" style={{ marginBottom: 20 }}>
          ...
          <button className="btn btn-ghost btn-large" onClick={populateStarterExercises}>Pré-remplir avec un exercice par thème</button>
      ...
      {showForm && <ExerciseFormPanel form={form} setForm={setForm} onSave={save} onCancel={() => setShowForm(false)} gameplan={gameplan} />}

      {!showForm && (
        <div className="roster-grid">
          {exercises.length === 0 && <div className="empty-state">Aucun exercice créé pour l'instant.</div>}
          {exercises.map((ex) => (
            <div className="roster-card" key={ex.id}>
              <div className="roster-card-info">
                <div className="roster-card-name">{ex.name}</div>
...
function ExerciseThumbnail({ diagram }) {
  const canvasRef = useRef(null);
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const w = canvas.clientWidth || 160, h = Math.round(w * (2 / 3));
  ...
  return (
    <div className="exercise-thumbnail-wrap">
      <PitchBackgroundFull />
...
function ExerciseBankScreen({ exercises, gameplan, specific, onAddToSession }) {
  ...
  grouped["_untagged"] = [];
  exercises.forEach((ex) => {
    if (!ex.gameplanSection) { grouped["_untagged"].push(ex); return; }
    if (specific && ex.gameplanChoice !== getGameplanValue(gameplan, ex.gameplanSection)) return;
    (grouped[ex.gameplanSection] || (grouped[ex.gameplanSection] = [])).push(ex);
  });
            ...
            <div className="roster-grid">
              {grouped[q.path].map((ex) => (
                <div className="roster-card" key={ex.id}>
                  ...
                  <div className="roster-card-info">
                    <div className="roster-card-name">{ex.name}</div>
                    <div className="roster-card-position">{ex.gameplanChoice}</div>
                    <div className="roster-card-usage">{ex.duree} min · {ex.nbJoueurs || "—"} joueurs</div>
                  </div>
                  <div className="roster-card-actions">
...
function SessionsScreen({ roster }) {
  ...
  function populateStarterSessions() {
    ...
    const newExercises = STARTER_EXERCISES.filter((ex) => !existingExNames.has(ex.name)).map((ex) => ({ id: newId(), ...emptyExerciseForm(), ...ex, createdAt: Date.now() }));
...
const CSS = `
  ...
  .exercise-thumbnail-wrap { position: relative; width: 70px; height: 47px; border-radius: 6px; overflow: hidden; flex-shrink: 0; }
