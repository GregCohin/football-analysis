...
function ExerciseCreationScreen({ exercises, setExercises, gameplan }) {
  ...
  function populateStarterExercises() {
    ...
    const toAdd = STARTER_EXERCISES.filter((ex) => !existingNames.has(ex.name)).map((ex) => ({ id: newId(), ...emptyExerciseForm(), ...ex, diagram: [], createdAt: Date.now() }));
    if (toAdd.length === 0) { alert("Tous les exercices de démarrage sont déjà présents dans ta banque."); return; }
...
function SessionsScreen({ roster }) {
  ...
  function populateStarterSessions() {
    ...
    const newExercises = STARTER_EXERCISES.filter((ex) => !existingExNames.has(ex.name)).map((ex) => ({ id: newId(), ...emptyExerciseForm(), ...ex, diagram: [], createdAt: Date.now() }));
