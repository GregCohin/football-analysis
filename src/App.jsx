...
const UNSCOPED_STORAGE_KEYS = new Set(["tf_teams", "tf_seasons", "tf_club_info", "tf_active_team", "tf_active_season"]);
...
// Lit une clé pour une équipe/saison précise, sans changer le contexte actif — utile pour la vue Club.
function readScopedKeyFor(baseKey, teamId, seasonId) {
  ...
  try { return window.localStorage.getItem(baseKey + suffix); } catch (e) { return null; }
}
...
(function installStorageScoping() {
  ...
  Storage.prototype.getItem = function (key) { return rawGetItem.call(this, scopedStorageKey(key)); };
  Storage.prototype.setItem = function (key, value) { return rawSetItem.call(this, scopedStorageKey(key), value); };
  Storage.prototype.removeItem = function (key) { return rawRemoveItem.call(this, scopedStorageKey(key)); };
})();
        ...
        <>
          ...
          <div className="table-scroll">
            <table className="stat-table">
              <thead><tr><th>Équipe</th><th>Effectif</th><th>Matchs Studio</th></tr></thead>
              <tbody>
                {teams.map((t) => {
                  const roster = (() => { try { return JSON.parse(readScopedKeyFor("tf_roster", t.id, getActiveSeasonId()) || "[]"); } catch (e) { return []; } })();
                  const matchesIdx = (() => { try { return JSON.parse(readScopedKeyFor("tf_matches_index", t.id, getActiveSeasonId()) || "[]"); } catch (e) { return []; } })();
                  return <tr key={t.id}><td>{t.name}</td><td>{roster.length}</td><td>{matchesIdx.length}</td></tr>;
                })}
              </tbody>
            </table>
          </div>
