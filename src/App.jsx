  ...
  return (
    <div>
      ...
      <div className="club-chip-row">
        ...
        {roster.map((p) => (
          <button key={p.id} className={`club-chip ${session.presence && session.presence[p.id] ? "" : ""}`} style={{ cursor: "pointer", borderColor: session.presence && session.presence[p.id] ? "var(--gold)" : undefined, color: session.presence && session.presence[p.id] ? "var(--gold)" : undefined }} onClick={() => togglePresence(p.id)}>
            {playerFullName(p)}
          </button>
        ))}
      </div>
