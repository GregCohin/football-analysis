  ...
  if (screen === "home" || !currentMatch) {
    return (
      <div className="home">
        ...
        {!showNewForm && (
          <button className="btn btn-primary btn-large" onClick={() => setShowNewForm(true)} style={{ marginBottom: 20 }}>+ Nouveau match observé</button>
        )}
            ...
            <table className="stat-table">
              ...
              <tbody>
                {ALL_EVENTS.map((ev) => (
                  <tr key={ev.key}>
                    <td>{ev.label}</td>
                    <td>{(collectiveTimes[`${ev.key}::us`] || []).length || ""}</td>
                    <td>{(collectiveTimes[`${ev.key}::opp`] || []).length || ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
