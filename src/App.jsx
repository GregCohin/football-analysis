...
const PAD_ELEMENT_TYPES = [
  { key: "playerA", label: "Joueur A", shape: "disc", color: "#E3B23C" },
  { key: "playerB", label: "Joueur B", shape: "disc", color: "#D6483F" },
  { key: "keeper", label: "Gardien", shape: "disc", color: "#5B8FD6" },
  { key: "cone", label: "Plot", shape: "triangle", color: "#FF8C00" },
  { key: "ball", label: "Ballon", shape: "ball", color: "#FFFFFF" },
  { key: "goal", label: "But/Zone", shape: "rect", color: "#FFFFFF" },
];
...
function TacticalPadEditor({ elements, setElements }) {
  ...
  function handleDown(e) {
    const { x, y } = getPos(e);
    if (tool === "text") { setTextPending({ x, y }); setTextValue(""); return; }
    if (isArrowTool) { setDrawing({ id: newId(), type: tool, x1: x, y1: y, x2: x, y2: y, color: tool === "arrowMove" ? "#5B8FD6" : "#E3B23C" }); return; }
  ...
  return (
    <div>
      ...
      <p className="hint">Choisis un élément puis clique sur le terrain pour le placer. Pour les flèches, clique-glisse. "Flèche course" est en pointillés, "Flèche passe" en trait plein.</p>
    </div>
  );
}
