  ...
  return (
    <div>
            ...
            {roster.map((p) => {
              ...
              return (
                <tr key={p.id}>
                  ...
                  <td><button className="btn btn-ghost btn-small" onClick={() => saveWellnessRow(p.id)}>Enregistrer</button></td>
                </tr>
              );
            })}
      ...
      <div className="table-scroll" style={{ marginTop: 10 }}>
        <table className="stat-table">
          <thead><tr><th>Date</th><th>Joueur</th><th>Contexte</th><th>RPE (0-10)</th></tr></thead>
          <tbody>
            {[...rpeLog].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 15).map((r) => {
              const p = roster.find((x) => x.id === r.playerId);
              return <tr key={r.id}><td>{formatDateFr(r.date)}</td><td>{p ? playerFullName(p) : "?"}</td><td>{r.context}</td><td>{r.rpe}</td></tr>;
            })}
            {rpeLog.length === 0 && <tr><td colSpan={4} className="empty-state">Aucun ressenti enregistré.</td></tr>}
          </tbody>
        </table>
      </div>
