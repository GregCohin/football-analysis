...
function ClubEventsScreen({ teams, categories }) {
  ...
  function addEvent() {
    ...
    persist([...events, form].sort((a, b) => new Date(a.date) - new Date(b.date)));
      ...
      <div className="scouting-list" style={{ marginTop: 10 }}>
        {events.map((e) => {
          ...
          return (
            <div className="scouting-card" key={e.id}>
              <div className="scouting-info">
                <div className="scouting-name">{e.name}</div>
                <div className="scouting-meta">{formatDateFr(e.date)}{e.endDate ? ` → ${formatDateFr(e.endDate)}` : ""}{location ? ` · ${location.name}` : ""}</div>
...
const LIBRARY_TYPES = ["Livre", "Article", "Vidéo", "Formation", "Podcast", "Autre"];
...
function BibliothequeScreen() {
  ...
  function openNew() { setForm(emptyLibraryEntry()); setEditingId(null); setShowForm(true); }
  ...
  const allThemes = [...new Set(entries.map((e) => e.theme).filter(Boolean))].sort();
  const filtered = themeFilter ? entries.filter((e) => e.theme === themeFilter) : entries;
  const grouped = {};
  ...
  return (
    <div className="stats-screen">
      ...
      {entries.length === 0 && <div className="empty-state">Aucune référence enregistrée pour l'instant.</div>}
