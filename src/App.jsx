...
function emptyClubEvent() {
  return {
    id: newId(), name: "", date: todayIso(), endDate: "", description: "",
...
function ClubEventsScreen({ teams, categories }) {
  ...
  function addEvent() {
    ...
    if (form.targetCategories.length === 0 && form.targetTeamIds.length === 0) { alert("Choisis au moins une catégorie ou une équipe à convoquer."); return; }
          ...
          return (
            <div className="scouting-card" key={e.id}>
              <div className="scouting-info">
                ...
                <div className="scouting-meta">{formatDateFr(e.date)}{e.endDate ? ` → ${formatDateFr(e.endDate)}` : ""}{location ? ` · ${location.name}` : ""}{e.seriesId ? " · se répète" : ""}</div>
...
function ExerciseDetailModal({ exercise, onClose, onEdit }) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="new-match-card" style={{ maxWidth: 640, width: "100%", maxHeight: "88vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
        ...
        {frames.length > 1 && (
          <div style={{ marginTop: 12 }}>
            <ExerciseAnimationPlayer frames={frames} />
          </div>
        )}
        {frames.length <= 1 && exercise.diagram && exercise.diagram.length > 0 && (
          <div style={{ marginTop: 12 }}>
            <ExerciseThumbnail diagram={exercise.diagram} large />
          </div>
        )}

        <p className="radar-note" style={{ marginTop: 14, marginBottom: 4, fontWeight: 600, color: "var(--ink)" }}>Objectif</p>
        <p style={{ marginTop: 0 }}>{exercise.objectif || "—"}</p>

        <div className="roster-physical-grid" style={{ marginTop: 10 }}>
          <p style={{ margin: 0 }}><strong>Durée :</strong> {exercise.duree} min</p>
          <p style={{ margin: 0 }}><strong>Joueurs :</strong> {exercise.nbJoueurs || "—"}</p>
        </div>
        <p style={{ marginTop: 6 }}><strong>Matériel :</strong> {exercise.materiel || "—"}</p>
    ...
    <div>
      ...
      <div className="scout-obs-add">
        ...
        <select value={pickExerciseId} onChange={(e) => setPickExerciseId(e.target.value)}>
          ...
          {exercises.map((ex) => <option key={ex.id} value={ex.id}>{ex.name} ({ex.duree} min)</option>)}
        </select>
