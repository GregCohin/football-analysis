            ...
            <div className="new-match-card" style={{ marginTop: 16 }}>
              {typeTab === "championnat" && (
                <div style={{ marginBottom: 14 }}>
                  <div className="hint" style={{ marginTop: 0 }}>Raccourci niveau (pré-remplit nom et niveau, modifiable ensuite) :</div>
                  <div className="tabs" style={{ marginTop: 6 }}>
                    {COMPETITION_LEVELS.map((l) => (
                      <button key={l.key} className="tab" onClick={() => { setNewName(l.label); setNewLevel(l.key); }}>{l.label}</button>
                    ))}
                  </div>
                </div>
              )}
              <label>Nom de la compétition
                <input type="text" placeholder={`ex. ${typeTab === "coupe" ? "Coupe de District" : typeTab === "tournoi" ? "Tournoi de Noël" : typeTab === "amical" ? "Stage estival" : "Championnat Départemental 2"}`} value={newName} onChange={(e) => setNewName(e.target.value)} autoFocus />
              </label>
