...
const CLUB_GROUPS = [
  { id: "general", label: "Général", subTabs: [{ id: "infos", label: "Informations" }] },
  {
...
function ClubScreen() {
  ...
  return (
    <div className="stats-screen">
      ...
      <p className="radar-note">Chaque équipe a son propre effectif, ses matchs, sa causerie, etc. — rien n'est partagé entre elles. Sélectionne l'équipe active depuis le menu, en haut à gauche.</p>
      <div className="scouting-list">
        {teams.map((t) => (
...
const ASSISTANT_SUGGESTIONS = [
  ...
  "Quelles sont mes alertes du jour ?",
...
function answerAssistantQuestion(question, ctx) {
  ...
  if (/bless/.test(q)) {
    ...
    return `${active.length} joueur${active.length > 1 ? "s" : ""} blessé${active.length > 1 ? "s" : ""} : ${active.map((i) => { const p = ctx.roster.find((r) => r.id === i.playerId); return p ? `${playerFullName(p)} (${i.type})` : "?"; }).join(", ")}.`;
  }

  if (/prochain(e)? match|prochaine rencontre/.test(q)) {
  ...
  if (/fatigue|vigilance/.test(q)) {
    if (ctx.fatigueRisks.length === 0) return "Aucun joueur en zone de vigilance charge/fatigue actuellement.";
    return `En zone de vigilance : ${ctx.fatigueRisks.map((f) => playerFullName(f.player)).join(", ")}.`;
  }
  ...
  return "Je ne sais pas encore répondre à ce type de question — pour l'instant je reconnais des sujets précis (blessures, prochain match, séance, buteurs, passeurs, alertes, classement, fatigue, scouting). Essaie une des suggestions ci-dessous, ou reformule.";
}
...
function AssistantScreen({ roster, matches }) {
  ...
  useEffect(() => {
    ...
    try { setCauseries(JSON.parse(localStorage.getItem("tf_causeries") || "[]")); } catch (e) {}
    ...
    setCtx({ roster, injuries, scouted, upcomingFixture, upcomingSession, leagueStats, fatigueRisks, standings, alerts });
  }, [roster, matches]);
  ...
  return (
    <div className="stats-screen">
      <div className="stats-screen-header">
        <div className="eyebrow">Assistant coaching</div>
        <h1>Assistant IA</h1>
        <p className="subtitle">Pose une question sur ton équipe active, ou tiens ton journal de bord — les réponses viennent de tes données déjà enregistrées dans l'app, pas d'un modèle de langage. Une base à brancher sur une vraie IA plus tard.</p>
      </div>
...
function MatchReportView({ match, report, signals, labelA, labelB, onSaveComment }) {
      ...
      {reportsSubTab === "mes" && (
        <>
          ...
          {exportMultiMode ? (
            ...
            <>
              {selectedMatch && report && signals && <MatchReportView match={selectedMatch} report={report} signals={signals} onSaveComment={saveComment} />}
