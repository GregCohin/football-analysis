...
function drawPadElement(ctx, el, w, h) {
  ...
  if (el.type === "arrowMove" || el.type === "arrowPass") {
    ctx.strokeStyle = el.color; ctx.fillStyle = el.color; ctx.lineWidth = 3;
    const x2 = el.x2 * w, y2 = el.y2 * h;
    if (el.type === "arrowMove") ctx.setLineDash([8, 6]); else ctx.setLineDash([]);
  ...
  } else if (def.shape === "zone") {
    ctx.setLineDash([6, 4]); ctx.strokeStyle = def.color; ctx.lineWidth = 2;
...
function TacticalPadEditor({ elements, setElements }) {
  ...
  const isArrowTool = tool === "arrowMove" || tool === "arrowPass";
  ...
  useEffect(() => { setArrowStart(null); setDrawing(null); }, [tool]);
  ...
  useEffect(() => {
    ...
    elements.forEach((el) => drawPadElement(ctx, el, size.w, size.h));
    if (drawing) drawPadElement(ctx, drawing, size.w, size.h);
    if (arrowStart) {
      ctx.fillStyle = tool === "arrowMove" ? "#5B8FD6" : "#E3B23C";
      ctx.beginPath(); ctx.arc(arrowStart.x * size.w, arrowStart.y * size.h, 5, 0, Math.PI * 2); ctx.fill();
    }
  }, [elements, drawing, size, arrowStart, tool]);

  function getPos(e) {
    const rect = canvasRef.current.getBoundingClientRect();
    return { x: (e.clientX - rect.left) / size.w, y: (e.clientY - rect.top) / size.h };
  }
  function handleClick(e) {
    ...
    if (isArrowTool) {
      if (!arrowStart) {
        ...
        setElements((prev) => [...prev, { id: newId(), type: tool, x1: arrowStart.x, y1: arrowStart.y, x2: x, y2: y, color: tool === "arrowMove" ? "#5B8FD6" : "#E3B23C" }]);
        setArrowStart(null);
        setDrawing(null);
      }
      return;
    ...
    setElements((prev) => [...prev, { id: newId(), type: tool, x, y, color: def ? def.color : "#fff" }]);
  }
  function handleMove(e) {
    if (!isArrowTool || !arrowStart) return;
    const { x, y } = getPos(e);
    setDrawing({ type: tool, x1: arrowStart.x, y1: arrowStart.y, x2: x, y2: y, color: tool === "arrowMove" ? "#5B8FD6" : "#E3B23C" });
  }
  function commitText() {
    ...
    <div>
      ...
      {isArrowTool && (
        <p className="hint" style={{ marginTop: 0, marginBottom: 8 }}>
          {arrowStart ? "Clique le point d'arrivée de la flèche." : "Clique le point de départ de la flèche."}
        </p>
      )}
      ...
      <p className="hint">Choisis un élément puis clique sur le terrain pour le placer. Pour les flèches, un clic pour le départ, un clic pour l'arrivée. "Flèche course" est en pointillés, "Flèche passe" en trait plein.</p>
    </div>
  );
}
