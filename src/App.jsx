...
const EVENT_TYPE_INFO = {
  preparation: { label: "Amical", color: "#5B8FD6" },
...
function loadCompetitionsData() {
  try {
    const raw = localStorage.getItem("tf_competitions");
    return raw ? { ...emptyCompetitionsData(), ...JSON.parse(raw) } : emptyCompetitionsData();
  } catch (e) {
...
function buildAccueilEvents(comp, sessions) {
  const events = [];
  comp.preparation.fixtures.forEach((f) => events.push({ date: f.date, type: "preparation", title: `vs ${f.opponent}`, fixture: f }));
  comp.championnat.fixtures.forEach((f) => events.push({ date: f.date, type: "championnat", title: `vs ${f.opponent}`, fixture: f }));
  comp.coupes.fixtures.forEach((f) => events.push({ date: f.date, type: "coupe", title: `vs ${f.opponent}`, fixture: f }));
...
function AccueilScreen({ matches, roster, setSection }) {
  ...
  const allFixtures = [...comp.preparation.fixtures, ...comp.championnat.fixtures, ...comp.coupes.fixtures];
  ...
  const standings = comp.championnat.standings.map((t) => ({ ...t, points: Number(t.wins || 0) * 3 + Number(t.draws || 0), gd: Number(t.goalsFor || 0) - Number(t.goalsAgainst || 0) }))
