        ...
        {staff.map((s) => (
          <div className="roster-card" key={s.id} onClick={() => openEdit(s)} style={{ cursor: "pointer" }}>
            <div className="roster-card-info">
              ...
              {s.diplomas && <div className="roster-card-meta">{s.diplomas}{s.diplomaExpiry ? ` — renouvellement le ${formatDateFr(s.diplomaExpiry)}` : ""}</div>}
            </div>
  ...
  return (
    <div>
      ...
      <div className="roster-grid" style={{ marginTop: 16 }}>
        ...
        {exercises.map((ex) => (
          <div className="roster-card" key={ex.id}>
            <ExerciseThumbnail diagram={ex.diagram} />
            <div className="roster-card-info">
              <div className="roster-card-name">{ex.name}</div>
              <div className="roster-card-position">{ex.objectif}</div>
              <div className="roster-card-usage">{ex.duree} min · {ex.nbJoueurs || "—"} joueurs</div>
            </div>
            <div className="roster-card-actions">
      ...
      {selectedId && selected && (
        <div>
          ...
          {DEV_THEMES.map((theme) => {
            ...
            return (
              <div key={theme.key} style={{ marginBottom: 22 }}>
                <div className="panel-heading" style={{ marginTop: 0 }}>{theme.label}</div>
...
export function emptyExerciseForm() {
  return { name: "", objectif: "", duree: 15, nbJoueurs: "", materiel: "", description: "", gameplanSection: "", gameplanChoice: "", gameplanLinks: [], diagram: [], diagramFrames: [], category: "tactique", ageFormat: "standard", fffBracket: "", fffCategory: "", curriculumFederation: "" };
}
...
function ExerciseCreationScreen({ exercises, setExercises, gameplan }) {
  ...
  return (
    <div>
      ...
      {!showForm && (
        <div className="roster-grid">
          {exercises.filter((e) => (e.category || "tactique") === categoryFilter && (e.ageFormat || "standard") === ageFormatFilter).length === 0 && <div className="empty-state">Aucun exercice {EXERCISE_CATEGORIES.find((c) => c.key === categoryFilter).label.toLowerCase()} créé pour l'instant dans ce format.</div>}
          {exercises.filter((e) => (e.category || "tactique") === categoryFilter && (e.ageFormat || "standard") === ageFormatFilter).map((ex) => (
            <div className="roster-card" key={ex.id}>
              <ExerciseThumbnail diagram={ex.diagram} />
              <div className="roster-card-info">
                <div className="roster-card-name">{ex.name}</div>
                <div className="roster-card-position">{ex.objectif}</div>
                <div className="roster-card-usage">{ex.duree} min · {ex.nbJoueurs || "—"} joueurs{ex.gameplanChoice ? ` · ${ex.gameplanChoice}` : ""}</div>
              </div>
              <div className="roster-card-actions">
                  ...
                  {byTheme[theme].map((ex) => (
                    <div className="roster-card" key={ex.id}>
                      ...
                      <div className="roster-card-info">
                        <div className="roster-card-name">{ex.name}</div>
                        <div className="roster-card-position">{ex.objectif}</div>
                        <div className="roster-card-usage">{ex.duree} min · {ex.nbJoueurs || "—"}</div>
                      </div>
                      <div className="roster-card-actions">
                        <button className="btn btn-ghost btn-small" onClick={() => setViewingExercise(ex)}>Voir</button>
            ...
            <div className="roster-grid">
              {grouped[q.path].map((ex) => {
                ...
                <div className="roster-card" key={ex.id}>
                  ...
                  <div className="roster-card-info">
                    <div className="roster-card-name">{ex.name}</div>
                    <div className="roster-card-position">{thisLink ? thisLink.choice : ex.gameplanChoice}{allLinks.length > 1 ? ` · +${allLinks.length - 1} autre${allLinks.length > 2 ? "s" : ""} thème${allLinks.length > 2 ? "s" : ""}` : ""}</div>
                    <div className="roster-card-usage">{ex.duree} min · {ex.nbJoueurs || "—"} joueurs</div>
                  </div>
                  <div className="roster-card-actions">
                    <button className="btn btn-ghost btn-small" onClick={() => setViewingExercise(ex)}>Voir</button>
