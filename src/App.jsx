...
function AdversairePlayersManager({ matches }) {
  ...
  const sorted = [...scouted].sort((a, b) => (a.club || "").localeCompare(b.club || "") || playerFullName(a).localeCompare(playerFullName(b)));
  ...
  return (
    <div>
      ...
      {!showForm && (
        <button className="btn btn-primary btn-large" onClick={openNewForm} style={{ marginBottom: 20 }}>+ Ajouter un joueur adverse</button>
      )}
      ...
      {showForm && (
        <div className="new-match-card">
          ...
          <label>Club<input type="text" placeholder="ex. FC Rival" value={form.club} onChange={(e) => setForm((f) => ({ ...f, club: e.target.value }))} /></label>
          <label>
            Poste
        ...
        {sorted.map((p) => (
          <div className="roster-card" key={p.id}>
            <div className="roster-card-info">
              ...
              <div className="roster-card-position">{p.club} · {p.position}</div>
  ...
  return (
    <div className="stats-screen">
      <div className="stats-screen-header">
        <div className="eyebrow">Assistant coaching</div>
        <h1>Scouting</h1>
        <p className="subtitle">Classement des joueurs adverses selon l'adéquation avec les profils de poste de ton Projet de jeu. Les fiches se gèrent depuis Effectifs → Adversaire.</p>
      </div>
