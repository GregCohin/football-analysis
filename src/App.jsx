  ...
  return (
    <div className="stats-screen">
      ...
      {scoutingSubTab === "interet" && (
        <>
          <p className="radar-note">Joueurs marqués "Intérêt fort" dans ce poste — ils restent visibles dans les autres onglets, ceci n'est qu'un filtre.</p>
          <div className="scouting-list">
...
function VideothequeScreen() {
  ...
  const filtered = isSpecialTab ? [] : clips.filter((c) => clipInSubTab(c, subTab) && clipMatchesSearch(c, searchQuery, roster, allFullMatches));
  const grouped = {};
  filtered.forEach((c) => {
  ...
  return (
    <div className="stats-screen">
      ...
      {playlist.length > 0 && !isSpecialTab && (
        <div className="home-actions-row" style={{ marginBottom: 16 }}>
          <button className="btn btn-primary btn-small" onClick={playPlaylist}>▶ Lire la playlist ({playlist.length})</button>
        </div>
      )}
      ...
      {!isSpecialTab && eventGroups.map((label) => (
        <div key={label} style={{ marginBottom: 24 }}>
          ...
          <div className="clip-grid">
            {grouped[label].map((c) => (
              <div className="clip-card" key={c.id} onClick={() => setEditingClip(c)}>
...
function InjuriesScreen({ roster, injuries, setInjuries, programs }) {
  ...
  function markHealed(id) {
    setInjuries((prev) => prev.map((i) => (i.id === id ? { ...i, status: "guéri", dateFinReelle: todayIso() } : i)));
  }
  function remove(id) {
            ...
            return (
              <div className="scouting-card" key={inj.id}>
                ...
                <div className="scouting-info">
                  ...
                  {inj.programPhases.length > 0 && <div className="scouting-meta">Réathlétisation : {doneCount}/{inj.programPhases.length} phases validées</div>}
                </div>
                <div className="roster-card-actions">
