...
function competitionLevelWeight(levelKey) {
  const found = COMPETITION_LEVELS.find((l) => l.key === levelKey);
    ...
    <div className="stats-screen">
            ...
            {inType.map((c) => (
              <div className="roster-card" key={c.id} onClick={() => setSelectedId(c.id)} style={{ cursor: "pointer" }}>
                <div className="roster-card-info">
                  ...
                  <div className="roster-card-position">{c.fixtures.length} rencontre{c.fixtures.length > 1 ? "s" : ""}{c.level && ` · ${COMPETITION_LEVELS.find((l) => l.key === c.level).label}`}</div>
                </div>
                <div className="roster-card-actions">
      ...
      {selected && (
        <div style={{ marginTop: 16 }}>
          ...
          <div className="panel-heading" style={{ marginTop: 10 }}>{selected.name}{selected.groupName && ` — ${selected.groupName}`}</div>
          <label style={{ display: "inline-block", marginBottom: 14 }}>
            Niveau de la compétition
            <select value={selected.level || ""} onChange={(e) => updateCompetition(selected.id, "level", e.target.value)} style={{ display: "block", marginTop: 4 }}>
              <option value="">Non renseigné</option>
              {COMPETITION_LEVELS.map((l) => <option key={l.key} value={l.key}>{l.label}</option>)}
            </select>
          </label>
...
function ScoutingScreen({ matches }) {
  ...
  const leagueStats = useMemo(() => computeLeaguePlayerStats(allFullMatches, roster, scouted), [allFullMatches, roster, scouted]);
