...
function TacticalPadEditor({ elements, setElements }) {
  ...
  function handleClick(e) {
    ...
    if (isRotateTool) {
      ...
      if (!target) return;
      setElements((prev) => prev.map((el) => (el.id === target.id ? { ...el, rotation: ((el.rotation || 0) + 15 + 360) % 360 } : el)));
      return;
    }
  ...
  return (
    <div>
      ...
      {isRotateTool && <p className="hint" style={{ marginTop: 0, marginBottom: 8 }}>Clic gauche sur un élément pour le pivoter (+15°), clic droit pour l'autre sens.</p>}
      {stepHint && <p className="hint" style={{ marginTop: 0, marginBottom: 8 }}>{stepHint}</p>}
      <div className="pad-wrap" ref={wrapRef}>
        <PitchBackgroundFull />
        <canvas ref={canvasRef} onClick={handleClick} onMouseMove={handleMove} onContextMenu={handleContextMenu} style={{ cursor: isRotateTool ? "pointer" : undefined }} />
        {textPending && (
      ...
      <p className="hint">Choisis un élément puis clique sur le terrain pour le placer. Pour les flèches et la zone, un clic par étape (coche "Courber" avant de commencer pour ajouter un point de courbure). "Flèche course" est en pointillés, "Flèche passe" en trait plein, "Flèche course + ballon" est ondulée. L'outil "Pivoter" oriente un élément déjà placé, clic droit pour l'autre sens.</p>
    </div>
  );
}
